...
1. 用户在 harness 里打 `/bridge <任务描述>` 触发 skill。任务描述可以是具体的（"这个报错怎么修"）也可以是模糊的（"帮我完善一下这个项目"）
2. **如果任务描述模糊（如"完善"、"改进"、"看看有什么问题"），agent 应主动读取当前项目代码库，总结项目结构、关键文件、现状**，把总结作为 CONTEXT 段内容打包。用户不需要手动提供全部上下文
