...
3. agent 用 Playwright MCP 自动执行发送循环（见下方"多轮对话循环"）
4. agent 把提取到的答案直接用于后续操作，不需要用户 `/bridge-back`

### 多轮对话循环（核心逻辑）

agent 发送首问包后，chat 可能会反问补充信息而不是直接给最终答案。agent 需要循环处理：

```
循环开始：
  1. browser_type 输入消息内容
  2. browser_press_key Enter 发送
  3. 等待 chat 生成完成（见下方"生成完成检测"）
  4. browser_evaluate 检查 sentinel 匹配数：
     - matches > 1 → chat 给了最终答案，提取最后一个 ===BRIDGE_ANSWER=== 段，退出循环
     - matches == 1 → chat 在反问，提取最后一段 AI 回答文本
  5. 如果是反问：
     - agent 自己决定怎么回答（基于项目上下文和首问包目标）
     - 把回答输入到输入框，回到步骤 2 继续循环
  6. 安全限制：最多循环 5 轮，超过后 fallback 到手动模式
```

### 生成完成检测

chat 流式生成时，sentinel 标记在回答最末尾才出现。检查太早会误判为"反问"。检测方法：

**方法 A（推荐）：轮询文本长度稳定**
```javascript
// 每 2 秒检查一次页面文本长度，连续 2 次不变 = 生成完成
() => document.body.innerText.length
```
agent 用 `browser_evaluate` 获取文本长度，等 2 秒再取一次，如果不变则生成完成。

**方法 B：查停止按钮**
生成中通常有"停止生成"按钮。agent 用 `browser_snapshot` 查看有没有"停止"/"Stop"按钮——有就说明还在生成，没有就说明完成。

**方法 C：固定等待**
简单但不够可靠——等 15-30 秒。复杂任务可能需要 60 秒以上。作为兜底方案。
...
() => {
  ...
  if (matches.length > 1) {
    // 严格模式：取最后一个匹配段（第一个是用户消息里的模板，最后一个是 chat 的答案）
    return 'STRICT:' + matches[matches.length - 1][0];
  }
  if (matches.length === 1) {
    // chat 在反问或没按格式回答——取最后一段 AI 消息
    const els = document.querySelectorAll('[class*="assistant"], [class*="answer"], [class*="response"], .markdown-body, [class*="message-content"]');
    let lastAi = '';
    for (const el of els) {
      const t = (el.textContent || '').trim();
      // 排除用户消息（含 sentinel 模板的那条）
      if (t.length > 50 && !t.includes('===BRIDGE_ANSWER=== <')) lastAi = t;
    }
    return 'LOOSE:' + lastAi;
  }
  return 'NO_ANSWER';
...
**返回值格式**：
- `STRICT:===BRIDGE_ANSWER===\n...\n===BRIDGE_ANSWER_END===` → 最终答案，提取 sentinel 间内容
- `LOOSE:<chat 的反问或无格式回答>` → 反问或没按格式，agent 判断是继续回答还是当最终答案用
- `NO_ANSWER` → 页面没内容，可能出错了
...
| 项目 | 实测结果 |
|------|---------|
| 输入框类型 | contenteditable div（不是 textarea），Vue 框架 |
| 输入方式 | `browser_type` 的 `fill()` 有效；**不要用 `browser_evaluate` 改 innerHTML**——Vue 响应式不触发，发不出去 |
| 发送方式 | `browser_press_key Enter` |
| 输入框定位 | `browser_snapshot` 找 `textbox` role 的元素 ref |
| sentinel 遵守 | Kimi 会按首问包里的 INSTRUCTIONS 段要求用 sentinel 格式输出最终答案 |
| 多轮对话 | Kimi 会反问补充信息，agent 回答后 Kimi 给最终答案（带 sentinel） |
| 生成时间 | 简单问题 ~8 秒，复杂方案 ~15-30 秒，需要轮询检测完成 |
...
| 中间多轮对话 | 在 chat 网页里自由多轮 | agent 自动处理多轮：检测反问 → 自动回答 → 等最终答案（最多 5 轮） |
